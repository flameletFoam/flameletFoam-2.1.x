...
if (Zeta != 0)
{
   ...
   for (int i=0;i<integratedData_[tableNames_["Z"]].size();i++)
   {
      ...
      if (varZ_[i] > 1e-7)
      {
          ...
          int    gridPoints = 200;
          List<scalar> hZ_(gridPoints, 0.0);
          List<scalar> helpZ_(gridPoints, 0.0);
          List<scalar> delta_(gridPoints, 0.0);
...
{
Info << "calculate sensible Enthalpy hs" << endl;
label labelT(tableNames_["T"]);
List<scalar> hs(singleData_[labelT].size(), 0.0);
...
for (int i=0;i<singleData_[labelT].size();i++)
{
   for (int j=0; j<thermo.composition().species().size();j++)
   {
      ...
      hs[i] += singleData_[j][i] * composition.Hs(k, singleData_[labelT][i]);
   }
}
singleData_.append(hs);
}
...
Foam::canteraReader::canteraReader(const IOdictionary& canteraDict, hsReactionThermo& thermo, basicMultiComponentMixture& composition)
